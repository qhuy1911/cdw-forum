...
public class ForumData {
    private static final ForumData instance = new ForumData();
    private Map<String, User> userData;
    private Map<Integer, Topic> topicData;
    ...
    private void init() {
        ...
        topicData = new HashMap<Integer, Topic>();
        Category cat1 = new Category("Học hành");

        userData.put("chuotcon", new User("chuotcon", "chuotcon", "lehoang@yahoo.com",
                new GregorianCalendar(2008, Calendar.FEBRUARY, 22).getTime()));
        userData.put("leb", new User("leb", "leb", "ntlan@yahoo.com",
                new GregorianCalendar(2007, Calendar.APRIL, 15).getTime()));
        userData.put("doctorQ", new User("doctorQ", "doctorQ", "mjohn@yahoo.com",
                new GregorianCalendar(2008, Calendar.FEBRUARY, 22).getTime()));
        userData.put("itss", new User("itss", "itss", "vovanthang@yahoo.com",
                new GregorianCalendar(2007, Calendar.AUGUST, 30).getTime()));
        userData.put("dt03", new User("dt03", "dt03", "dotuan@hotmail.com",
                new GregorianCalendar(2007, Calendar.JANUARY, 7).getTime()));
        userData.put("GSKH", new User("GSKH", "GSKH", "giasu@yahoo.com",
                new GregorianCalendar(2007, Calendar.JULY, 14).getTime()));


        Topic topic1 = new Topic(1, "Chuyện học phí!!!",
                "Từ lớp mẫu giáo đến lớp 12-trường công-Tại sao lại phải đóng học phí?!",
                userData.get("doctorQ"), cat1);
        ...
        Topic topic2 = new Topic(2, "Thủ tướng: Trường ĐH tự quyết mức học phí",
                "ọc phí mầm non và phổ thông ở trường công lập là sự đóng góp một phần nhỏ " +
                        ...
                        "phí đào tạo", userData.get("leb"), cat1);
        ...
        Topic topic3 = new Topic(3, "Học phí trái buổi",
                "Cháu học trường THPT Trần khai nguyên Q5....Lớp 12\n" +
                        ...
                        "năm ngóai (70.000đ)", userData.get("itss"), cat1);
        Topic topic4 = new Topic(4, "Một số mốc quan trọng trong đề án \"Tăng học phí\"",
                "# Ngày 4/9, trước khai giảng một ngày, Thủ tướng Chính phủ ban hành chỉ thị cho sinh viên vay vốn học tập với tinh thần " +
                        "\"không để sinh viên bỏ học vì không có tiền đóng học phí\". ", userData.get("dt03"), cat1);

        topic4.addMessage(new Message("Re: Một số mốc quan trọng trong đề án \"Tăng học phí\"",
                "Tạm hoãn lại rồi.", userData.get("GSKH")));
        ...
        topic5.addMessage(new Message("Re: Xung quanh dự thảo của Đề án tăng học phí",
                ...
                userData.get("itss")));
        topic5.addMessage(new Message("Re: Xung quanh dự thảo của Đề án tăng học phí",
                "Sẽ làm nước lạc hậu hơn",
                userData.get("GSKH")));

        topicData.put(1, topic1);
        topicData.put(2, topic2);
        topicData.put(3, topic3);
        topicData.put(4, topic4);
        topicData.put(5, topic5);
    }

    public Collection<Topic> getTopics() {
        return topicData.values();
    }
    ...
    public Topic findTopic(Integer id){
        return topicData.get(id);
    }

    public void addTopic(Topic topic) {
        topicData.put(topic.getId(), topic);
    }
